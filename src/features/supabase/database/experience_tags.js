...
// Fetch the tags of the experience
const fetchExperienceTags = async ({
  // Experience ID
  experienceID,
}) => {
  const { data, error } = await supabase.from('experiences_tags').select(`tag_id, tags (name)`).eq('experience_id', experienceID);
  return { data, error };
}
